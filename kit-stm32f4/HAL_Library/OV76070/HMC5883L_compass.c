...
int HMC5883L_data()
{
      ...
      com_z = (signed short)dec16(UC[2],UC[3]);    
      com_y = (signed short)dec16(UC[4],UC[5]);
      head = atan2(com_y,com_x);
      sprintf(debug,"Head = %02f [Hx: %d, Hy: %d Hz: %d]\n\r",head*180/PHI, com_y, com_z);
      usart_send((uint8_t*)debug,strlen(debug));
